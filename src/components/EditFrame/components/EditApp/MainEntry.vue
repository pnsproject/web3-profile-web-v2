    ...
    <div class="listed">
      ...
      <Draggable v-model="editResult" item-key="id" ghost-class="ghost" drag-class="drag" :animation="150">
        <template #item="{ element }">
          <div class="item">
            <div class="info">
              <img :src="element.icon" :alt="element.website_name">
              <div class="name">{{ element.website_name }}</div>
            </div>
      ...
      <div class="unlisted">
        <div v-for="item in editResult" :key="item.id" class="item" >
          <div class="info">
            ...
            <div class="name">{{ item.website_name }}</div>
          </div>
          <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const editResult = ref<Global.App[]>(editWindows.option.data)
